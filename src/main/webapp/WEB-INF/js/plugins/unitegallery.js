...
function UGSlider(){
	...
	this.setItem = function(objItem, forseTransition, role){
		...
		//customized call give like
		setTimeout(function(){
			$(".connUs-ptosGiveLike").off("click").on("click",function(events){
				photosClass.givePhotoLike(events);
			});
			
			$(".connUs-ptosViewMoreLikes").off("click").on("click",function(events){
				photosClass.getPhotoLikes(events);
			});
			
			$(".connUs-ptosAddcomnts-btn").off("click").on("click",function(events){
				photosClass.addPhotoCmnts(events);
			});
			
			$(".connUs-ptosViewMoreCmnts").off("click").on("click",function(events){
				photosClass.getPhotoCmnts(events);
			});
		},1000);
	}
	
	//customized to set Img slide data position
	function customImgSildeDataSetPosition(){
		var itemWrapperHeight = ($(".ug-item-wrapper").outerHeight() + 5);
		$(".connUs-imgSliderDesc").css("top", itemWrapperHeight + "px");
	}
	...
	this.placeNabourItems = function(){
		...
		//customized for like and comment container
		$('.ug-lightbox-top-panel .connUs-imgSliderDesc').detach();
		var htmlDom = $($('.connUs-imgSliderDesc')[currentIndex]).clone();
		$(".ug-lightbox-top-panel").append(htmlDom);
		customImgSildeDataSetPosition();
	...
	this.setSize = function(width, height){
		...
		//set slide wrapper
		 ...
		 //customized for imgSilderDescHeight position
		 var imgSilderDescHeight = $(".connUs-imgSliderDesc").outerHeight();
		 itemHeight = itemHeight - (imgSilderDescHeight + 10);
...
function UGLightbox(){
	...
	function updateTextPanelText(currentItem){
		...
		//customized for imgDesc
		g_objTextPanel.setTextPlain(currentItem.description, currentItem.description);
	}
	...
	this.open = function(index){
		...
		if(g_objTextPanel){
			...
			//customized for imgDesc
			g_objTextPanel.setTextPlain(objItem.description, objItem.description);
		}

...
<script id="cmntsListData-hb-template" type="text/x-handlebars-template">
{{#if data}}
	{{#if data.likeOrCmntsList}}
        {{#each data.likeOrCmntsList}}
	    <div class="connUs-cmntsOrLike-cont" cmntsUserId="{{userId}}">
		   ...
		   <div class="connUs-data-cont">
		   		<div class="connUs-cmntsOrLike-name">{{#userIsYou _id userName}}{{/userIsYou}}</div>
		   		<div class="connUs-cmnts-cmntTxt">{{{comments}}}</div>
		   		<div class="connUs-cmnts-DateAndTime">{{dateAndTime}}</div>
		   </div>
	    </div>
        {{/each}}
    {{else}}
        <div class="connUs-noCmntsOrLikeTxt">No Comments Yet!</div>
	{{/if}}
{{/if}}
</script>
</body>
